...
fn main() -> io::Result<()> {
	...
	let mut dest: Point = Point {x: 0, y: 0};
	...
	println!("Going to ({}, {})", dest.x, dest.y);

	println!("Nb steps = {}", explore(&map, current, &dest, width, height));

	Ok(())
}


fn explore(map: &Vec<Vec<char>>, pos: Point, dest: & Point, width: usize, height: usize) -> usize
{
