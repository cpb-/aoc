...
fn main() -> io::Result<()> {
	...
	loop {
		if ! try_position(Point {x:500, y:0}, &polylines, &mut sands, max_y + 2) {
			break;
		}
...
fn try_position(p: Point, rock: &Vec<Vec<Point>>, sand: &mut Vec<Point>, max_y: u32) -> bool {
	...
	if occuped_cell(&pos, rock, sand, max_y) {
		return false;
	}
...
fn occuped_cell(p: &Point, rock: &Vec<Vec<Point>>, sand: &Vec<Point>, max_y: u32) -> bool {
