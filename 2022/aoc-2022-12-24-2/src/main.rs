...
fn main() -> io::Result<()> {
	...
	let minute = search_path(&grid, 1, 0, grid[0].len() - 2, grid.len() - 1, 0);
	println!("Exit found after {} minutes", minute);
	let minute = search_path(&grid, grid[0].len() - 2, grid.len() - 1, 1, 0, minute);
	println!("Back after {} minutes", minute);
	let minute = search_path(&grid, 1, 0, grid[0].len() - 2, grid.len() - 1, minute);
	println!("Exit again found after {} minutes", minute);
	
	Ok(())
}


fn search_path(grid: &Vec<Vec<char>>, startx: usize, starty: usize, endx: usize, endy: usize, start_time: usize) -> usize {

	let mut test_queue: VecDeque<(usize, usize, usize)> = VecDeque::new(); // (x, y, minute)
	test_queue.push_back((startx, starty, start_time));

	let mut history: HashSet<(usize, usize, usize)> = HashSet::new();
	history.insert((startx, starty, start_time));
