...
fn main() -> io::Result<()> {
	...
	for line in reader.lines() {
		...
		if words.len() == 2 {
			let num = words[1].parse::<i64>().unwrap();
	...
	loop {
		if known_values.contains_key("root") { break; }
		if equations.len() == 0 { break; }

		let eq = equations.remove(0);

//		println!("Trying {} {} {} -> {}", eq.1, eq.2, eq.3, eq.0);
